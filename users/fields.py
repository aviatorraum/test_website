...
class PasswordLengthValidator(object):
    ...
    def __call__(self, value):
        if self.min_length and len(value) < self.min_length:
            ...
            raise forms.ValidationError(
                _('Password too long must lesss %s characters ') % self.max_length,
                code=self.code)

pwd_length_validator = PasswordLengthValidator()
...
class LengthValidator(object):
    ...
    def __call__(self, value):
        if self.max_length and len(value) > self.max_length:
            raise forms.ValidationError(
                _('Text must less than %s characters ') % self.max_length,
                code=self.code)
...
class PasswordField(forms.CharField):
    ...
    default_validators = [pwd_length_validator, complexity_validator, ]
...
class UserTextField(forms.CharField):
